(function () {
  ...
  fetch("https://yourusername.github.io/your-repo-name/config.json") // 🔁 URL बदल्नुस्
    .then((res) => res.json())
    .then((config) => {
      if (config.active_websites.includes(currentWebsite)) {
        initPopup(); // ✅ यदि वेबसाइट allowed छ भने popup देखाउनु
      } else {
        console.log("Widget deactivated for this site.");
      }
    })
    .catch((err) => {
